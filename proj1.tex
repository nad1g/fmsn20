...
The mean of the spatial data is modelled as a linear function of the covariates. Figure \ref{fig:covariates} shows a plot of the mean temperature (observations) as a function of the available covariates.
\begin{figure}[ht]
	\includegraphics[width=0.8\linewidth]{covariates.png}
	\caption{Observation vs covariates}
	\label{fig:covariates}
\end{figure}
The covariates that have a linear relationship are the latitude ($\bx_1$), elevation ($\bx_2$) and distance to Swedish coast ($\bx_3$). Thus the mean function is expressed as
\begin{equation}
  \bmu = \beta_0 + \beta_1 \bx_1 + \beta_2 \bx_2 + \beta_3 \bx_3
\end{equation}
where $\beta_0$ is the intercept and $\beta_1 \dots \beta_3$ are the regressors. With $N = 250$ observations from the known locations, the estimated parameters (and their standard deviations ) are tabulated below.
\begin{table}[H]
\begin{tabular}{lcc}
Parameter & Estimate & SE \\
$\beta_0$ & 24.553445 & 0.928379 \\
$\beta_1$ & -0.177836 & 0.015600\\
$\beta_2$ & -0.004178 & 0.000455\\
$\beta_3$ & -0.092380 &  0.015663\\
\end{tabular}
\end{table}
The variance of the residuals $\sigmaeps^2 = 0.550609$.
